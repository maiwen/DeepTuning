...
class Lustre(Env):
    ...
    def _get_llite_pi(self, llite_path):
        ...
        with open(os.path.join(llite_path, 'stats'), 'r') as statsfile:
            ...
            if len(old_stats) <= 1:
                ...
                stats['reward'] = 5 * (read_throuput - old_stats['read_throuput']) + (write_throuput - old_stats['write_throuput']) + \
                                       5 * (read_ops - old_stats['read_ops']) + ( write_ops - old_stats['write_ops'])
    ...
    def reset(self):
        ...
        for param in _PARAMS[:3]:
            os.popen(set_osc + param['name'] + '=' + param['initial'])
        for param in _PARAMS[3:7]:
            os.popen(set_llite + param['name'] + '=' + param['initial'])
