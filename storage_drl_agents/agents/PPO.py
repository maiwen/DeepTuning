...
class PPO(Agent):
    ...
    def __init__(self, env, state_dim, action_dim,
        ...
        self.actor_hidden_size = self.state_dim * 2 + 1
        self.critic_hidden_size = self.state_dim * 2 + 1
    ...
    def train(self):
        ...
        states_var = self.normal(to_tensor_var(batch.states, self.use_cuda).view(-1, self.state_dim))
    ...
    def save(self, filename):
        th.save(self.actor.state_dict(), 'models/actor_'+filename+'_model.pkl')
        th.save(self.critic.state_dict(), 'models/critic_' + filename+'_model.pkl')

    def load(self, filename):
        if os.path.isfile('models/actor_'+filename+'_model.pkl'):
            actor_state_dict = th.load('models/actor_'+ filename+'_model.pkl', map_location=lambda storage, loc: storage)
            self.actor.load_state_dict(actor_state_dict)
        if os.path.isfile('models/critic_' + filename+'_model.pkl'):
            critic_state_dict = th.load('models/critic_' + filename+'_model.pkl', map_location=lambda storage, loc: storage)
